<template>
	<div>
		...
		<el-table :data="ords" border style="width: 100%">
			...
			<el-table-column fixed prop="addr" label="收货地址" width="240">
			</el-table-column>
			<el-table-column fixed prop="phone" label="电话" width="180">
			</el-table-column>
			<el-table-column fixed prop="shipno" label="发货单号" width="150">
			</el-table-column>
			<el-table-column fixed prop="amount" label="总价" width="160">
			</el-table-column>
			<el-table-column fixed prop="tcreate" label="下单时间" width="190">
			</el-table-column>
			<el-table-column fixed prop="ispay" label="是否付款" width="140">
			</el-table-column>
			<el-table-column fixed prop="scope" label="操作" width="200">
		...
		<div style="display: flex;">
			  <el-button type="primary" @click="more()" plain>显示更多</el-button>
		</div>
	</div>
</template>
...
<script>
	export default{
		...
		methods:{
			detailed(ordid){
				let _this = this
				this.$get("/api/adm/ord/findByOid/"+ordid+"").then(function(resp){
			...
			more(){
				...
				let _this = this;
				this.$get("/api/adm/ord/findAll/"+this.page+"/6").then(function(resp){
					var ord = resp.data.data
					for(var i=0;i<ord.length;i++){
						_this.ords.push(ord[i]);
					}
